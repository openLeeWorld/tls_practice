...
mpz_class private_key; // 인증서 확인 메시지에서 사용될 ECDSA 인증서의 비밀키
static string init_certificate() {
	ifstream f("../ec_cert.pem"); //openssl req -x509 -new -sha256 -key ec_key.pem -out ec_cert.pem
	vector<unsigned char> r;
	for(string s; (s = get_certificate_core(f)) != "";) { // 인증서 체인의 모든 인증서에 대해 반복
		auto v = base64_decode(s);
		for(int i=0; i<3; i++) r.push_back(0);
		mpz2bnd(v.size(), r.end() - 3, r.end()); // 인증서 크기 삽입
		r.insert(r.end(), v.begin(), v.end()); // DER 인증서 삽입
	}
	r.push_back(0); r.push_back(0);
    const int REQUEST_CONTEXT = 0; // 1.3버전에서 추가된 필드
	vector<uint8_t> v = {0x16,3,3,0,0, CERTIFICATE, 0, 0, 0, REQUEST_CONTEXT, 0, 0, 0};
	mpz2bnd(r.size(), v.end() - 3, v.end()); // 헤더의 마지막 0,0,0 위치
	mpz2bnd(r.size() + 4, v.begin() + 6, v.begin() + 9); // 중간 0,0,0 위치
	mpz2bnd(r.size() + 8, v.begin() + 3, v.begin() + 5); // 맨 앞 0,0
	r.insert(r.begin(), v.begin(), v.end()); // 헤더 삽입

	ifstream f2{"../ec_key.pem"}; // openssl ecparam -name secp256r1 -genkey -out ec_key.pem
	get_certificate_core(f2);//pass 
	auto jv = pem2json(f2);
	private_key = str2mpz(jv[0][1].asString()); // 비밀키 세팅
	return {r.begin(), r.end()}; // 인증서 리턴
}
// 인증서 메시지의 초기화
...
template<bool SV> string TLS13<SV>::client_ext() {
	...
	mpz2bnd(this->P_.x, ext.x, ext.x + 32); // secp256r1 좌표 세팅
	mpz2bnd(this->P_.y, ext.y, ext.y + 32); // P_는 TLS 1.2 클래스의 멤버
	mpz2bnd(sizeof(Ext) - 2, ext.extension_length, ext.extension_length + 2);
	mpz2bnd(this->prv_key_, prv_, prv_ + 32);
	curve25519_mul_g(ext.x2, prv_); // x25519 x좌표 세팅
...
{//check extension and return if it is capable of negotiating with us
	...
	return true;
} // extension을 체크해 우리와 1.3으로 통신할 수 있는지 확인
...
template<bool SV> string TLS13<SV>::server_ext() {
	...
	if(this->P_.x == -1) { //secp256r1이 아닌 경우
...
//	if(SV == (s == "")) protect_data();//SV send, client receive 
	if(s == "") return fin; // Finish 메시지를 생성하는 경우
	else return s == fin ? "" : this->alert(2, 51); // 분석하는 경우
}
...
{//debug // 포인터의 위치는 익스텐션의 시작 위치를 가리켜야 한다.
	int total_len = *p++ * 0x100 + *p++;
...
template<bool SV> string TLS13<SV>::client_hello(string &&s)
{
	if constexpr(SV) {//is not returning correct ext_start
		...
		p += *p + 1; // 세션 아이디 뒤로 포인터 이동
		int cipher_suite_len = *p++ * 0x100 + *p++;
		p += cipher_suite_len; // 사이퍼 수트 뒤로 포인터 이동
		p += *p + 1;//compression length 뒤로 이동하면 익스텐션의 시작점
		int ext_start = p - (unsigned char*)&s[0]; // TLS 1.2로 클라이언트 헬로 함수를 부른다.
...
{//handshake according to compromised version
		...
		if constexpr(SV) {
			...
			if(s = server_hello(); premaster_secret_) {
				...
				if(s = this->alert(2, 0); !(a = read_f()) || !(a = this->decode(move(*a))) ||
					(protect_data(), false) ||	(s = finished(move(*a))) != "") break; // 애플리케이션 암호화 준비 완료
			} else { // TLS 1.2
				...
				s = this->change_cipher_spec();
				s += TLS<SV>::finished(); // 1.2 버전의 finished 함수를 호출
	...
	if(s != "") { // break로 중간에 핸드셰이킹을 중단한 경우 
		write_f(s);//send alert message
		return false;
	} else return true; // 핸드셰이킹이 성공적으로 완료된 경우
}
...
{ // 1.3버전일 경우 premaster_secret_ 멤버변수가 세팅돼 있다.
...
template<bool SV> string TLS13<SV>::encode13(string &&s, int type)
{
	...
	string frag = s.substr(0, chunk_size) + string{type}; // type 패딩
	h1.set_length(frag.size() + 16);

	uint8_t *p = (uint8_t*)&h1; 
	this->aes_[SV].aad(p, 5);
	p = (uint8_t*)frag.data();
	this->aes_[SV].xor_with_iv(seq); // iv 값을 시퀀스 넘버와 xor
	auto tag = this->aes_[SV].encrypt(p, frag.size());
	this->aes_[SV].xor_with_iv(seq); // iv값을 원상복귀
...
template<bool SV> optional<string> TLS13<SV>::decode13(string &&s) 
{
	...
	this->aes_[!SV].xor_with_iv(seq); //iv복귀
	if(equal(auth.begin(), auth.end(), p->encrypted_msg + msg_len)) { // 인증 태그 확인 성공 시
